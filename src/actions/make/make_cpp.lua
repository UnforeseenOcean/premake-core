...
--
	...
	function cpp.initialize()
		rule 'cpp'
			...
			buildmessage  '$(notdir $<)'
			buildcommands {'$(CXX) $(%{premake.make.cpp.fileFlags(cfg, file)}) $(FORCE_INCLUDE) -o "$@" -MF "$(@:%.o=%.d)" -c "$<"'}

		rule 'cc'
			fileExtension {".c", ".s", ".m"}
			buildoutputs  {'$(OBJDIR)/%{file.objname}.o'}
			buildmessage  '$(notdir $<)'
			buildcommands {'$(CC) $(%{premake.make.cpp.fileFlags(cfg, file)}) $(FORCE_INCLUDE) -o "$@" -MF "$(@:%.o=%.d)" -c "$<"'}
	...
	function cpp.createRuleTable(prj)
		...
		for _, name in ipairs(usedRules) do
			local rule = premake.global.getRule(name)
...
--
	...
	function cpp.outputConfigurationSection(prj)
		...
		_p('# #############################################')
		_p('')
		make.outputSection(prj, cpp.elements.configuration)
	end
	...
	function cpp.defines(cfg, toolset)
		p.outln('DEFINES +=' .. make.list(table.join(toolset.getdefines(cfg.defines, cfg), toolset.getundefines(cfg.undefines))))
	end


	function cpp.includes(cfg, toolset)
		local includes = toolset.getincludedirs(cfg, cfg.includedirs, cfg.sysincludedirs)
		p.outln('INCLUDES +=' .. make.list(includes))
	end
	...
	function cpp.cppFlags(cfg, toolset)
		local flags = make.list(toolset.getcppflags(cfg))
		p.outln('ALL_CPPFLAGS += $(CPPFLAGS)' .. flags .. ' $(DEFINES) $(INCLUDES)')
	end


	function cpp.cFlags(cfg, toolset)
		local flags = make.list(table.join(toolset.getcflags(cfg), cfg.buildoptions))
	...
	function cpp.ldFlags(cfg, toolset)
		...
		p.outln('ALL_LDFLAGS += $(LDFLAGS)' .. make.list(flags))
	end
...
--
	...
	function cpp.perFileFlags(cfg, toolset, fcfg)
		...
		if fcfg.includedirs or fcfg.sysincludedirs then
			...
			if #includes > 0 then
				value = value ..  make.list(includes)
			end
		end
...
--
	...
	function cpp.outputFilesSection(prj)
		...
		make.outputSection(prj, cpp.elements.filesets)
	end
